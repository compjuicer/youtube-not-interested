...
// @version     1.0
// @author      -
// @description 2/20/2021, 12:27:33 AM
// ==/UserScript==
...
(function(){
  ...
  const ME = "ytd-one-click-dismiss";
  ...
  async function onClick(e) {
    ...
    try {
      const index = STYLE.sheet.insertRule(
        "ytd-menu-popup-renderer { display: none !important }"
      );
      ...
      for (const el of document.querySelector(
        'ytd-menu-popup-renderer [role="listbox"]'
      ).children) {
      ...
      const { x, y } = e;
      const box = $(document.elementsFromPoint(x, y)).filter('ytd-rich-item-renderer');
      box.remove();
  ...
  function getProp(obj, path) {
    return path.split(".").reduce((res, name) => res && res[name], obj);
  }
